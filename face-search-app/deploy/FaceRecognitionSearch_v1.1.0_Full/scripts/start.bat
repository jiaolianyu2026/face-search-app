...
if not exist "%APP_ROOT%\backend\venv_new" (
    if not exist "%APP_ROOT%\backend\venv" (
        echo [错误] 虚拟环境不存在
        echo 应用根目录: %APP_ROOT%
        echo 请确保安装包完整，backend\venv_new 或 backend\venv 目录应该存在
        echo.
...
if not exist "%APP_ROOT%\frontend\node_modules" (
    ...
    echo.
    echo 建议：运行 scripts\install_frontend_deps.bat 安装前端依赖
    echo.
    pause
    exit /b 1
)
...
:: 启动前端服务
echo [2/2] 启动前端服务...
start "人脸识别前端" cmd /k "cd /d "%APP_ROOT%\frontend" && npx vite"
timeout /t 3 /nobreak >nul
